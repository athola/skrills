...
The HTTP transport serves the MCP protocol alongside a browser dashboard (Leptos SSR) and REST API endpoints for skills, metrics, and MCP server discovery (`/api/mcp-servers`). Routes are merged into a single Axum router with shared middleware for auth, CORS, and request IDs.
