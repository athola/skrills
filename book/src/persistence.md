...
- **Skill Mirrors**: Optional skill copies are stored in `~/.codex/skills-mirror/`. They are populated from `~/.claude/` when `sync-from-claude` or `skrills sync` runs, ensuring synchronization between Codex/Claude skill sources without modifying the originals.
...
- `sync-from-claude` (MCP tool): Populates the `skills-mirror` directory and reports on copied versus skipped files. Re-run it after any modifications to Claude skills to maintain synchronization.
