...
- **Fix: `FieldSupport` gains a direction, ending silent no-op syncs**: `FieldSupport` had no read/write axis and the orchestrator consulted the target's copy only, so one flag meant "reads it" for Claude and "receives it" for Cursor. A Cursor-to-Claude plugin-asset sync passed the gate, read nothing, wrote nothing, and reported success. `AgentAdapter` adds `read_support()` and `write_support()`, both defaulting to `supported_fields()`. Claude declares no plugin-asset writer and Cursor no plugin-asset reader. The orchestrator gates the read on the source and the write on the target, and records `SkipReason::UnsupportedField` naming the end that could not do it.
...
- **Fix: One unreadable file emptied skill discovery**: A `file_hash` failure aborted the whole parallel scan, so a single file vanishing mid-walk (routine while `~/.claude/plugins/cache` is rewritten) returned zero skills and the dashboard showed an empty list. The entry is now skipped with a warning, matching the frontmatter read below it. No test covers this: the failure needs a file to disappear between the walk and the stat, and the walker filters dangling symlinks before they reach `file_hash`.
