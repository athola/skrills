...
- **Feature: Cold-Window Real-Time Analysis (`skrills cold-window`)**: New continuously-refreshing analysis surface for the skrills ecosystem. Adds a `skrills-snapshot` wire-format crate and a `cold_window` module across `skrills-analyze`, `skrills-intelligence`, `skrills-metrics`, `skrills-tome`, `skrills-dashboard`, and `skrills-server`. Each tick re-reads authoritative state, runs a 4-tier alert policy with hysteresis and min-dwell (CAS-inspired tier model + ISA-18.2 alarm-management patterns), ranks hints via a recency-weighted multi-signal scorer, attributes tokens per-source (skill / plugin / MCP / conversation), and gates external research fetches through an AlertManager-style dispatcher with restart-resilient quota persistence at `~/.skrills/research-quota.json`. Browser surface ships in v0.8.0 (`--browser` flag); TUI panes are implemented as library code with 49 passing tests and mount in a follow-up. Token thresholds are research-backed: 20K = Advisory (Anthropic API quadratic inflection per the Feb 2026 HN "Expensively Quadratic" analysis), 50K = Caution (Willison "too many MCPs" overhead range), 80% budget = Warning, 100% = kill-switch engaged. Defense-in-depth XSS posture: server-side `html_escape` + browser-side `DOMParser` + `replaceChildren`. Adaptive cadence borrows the Linux scheduler's 0.7 / 0.9 load-ratio thresholds. 178 cold-window-specific tests + 2 doctests; full brief + spec + plan + war-room artifacts under `docs/cold-window-*.md`.
