...
- **Known: `rmcp` 0.10.0 remains on RUSTSEC-2026-0189** (severity 8.8, DNS rebinding in the Streamable HTTP server transport). `crates/server/src/http_transport.rs` mounts `StreamableHttpService`, the affected component, and applies no Host-header allowlist of its own, so the advisory applies to `skrills serve --http`. The fix needs rmcp >= 1.4, whose `Tool` and `CallToolResult` are `#[non_exhaustive]`: roughly 90 struct literals across nine files must move to `Tool::new_with_raw(..)` / `CallToolResult::success(..)` constructors. Tracked separately rather than folded into this branch. Worth noting for that work: rmcp 1.8's `StreamableHttpServerConfig::default()` ships a restrictive `allowed_hosts` of localhost/127.0.0.1/::1, so the upgrade closes the hole without extra configuration, but binding the server to a LAN address will then need an explicit `with_allowed_hosts`.
